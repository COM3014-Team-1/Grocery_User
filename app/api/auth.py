...
import jwt
from app.extensions import db, limiter  # Use shared extensions to avoid circular imports
...
def is_valid_password(password: str) -> bool:
    # Enforce at least 8 characters, one uppercase, one lowercase, one digit, and one special character.
    if len(password) < 8:
        return False
...
# In-memory storage to track failed login attempts.
failed_logins = {}
...
def is_account_locked(identifier: str) -> (bool, str):
...
def record_failed_attempt(identifier: str):
    ...
    record = failed_logins.get(identifier)
    if record:
        # Reset counter if last attempt is outside the attempt window.
...
# Rate limit the register endpoint to prevent abuse (5 per minute).
...
def register():
    ...
    if not is_valid_password(password):
        return jsonify({"error": "Password must be at least 8 characters long and include uppercase letters, lowercase letters, digits, and special characters."}), 400
...
# Rate limit the login endpoint to prevent brute force attempts (5 per minute).
